...
export function ProjectTaskList({
  ...
  const handleTaskStatusChange = (taskId: string, checked: boolean) => {
    setTasks(
      tasks.map((task) =>
        task.id === taskId
          ? { ...task, status: checked ? "completed" : "todo" }
          : task
      )
    );
  };
  ...
  return (
    <div className="divide-y bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-950">
      {filteredTasks.length === 0 ? (
        ...
        filteredTasks.map((task) => (
          <div
            ...
            onClick={() =>
              router.push(`/dashboard/project/${projectId}/task/${task.id}`)
            }
          >
